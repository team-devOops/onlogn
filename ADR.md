...
## ADR-021: 공개 프로필 월간 캘린더 강도는 완료 task(done) 기준으로 집계

- **상태**: 승인
- **결정**: `profile.html`의 월간 활동 히트맵은 날짜별 전체 할 일 수가 아니라 `status=done`인 완료 건수만 누적해 색 강도를 계산한다.
- **근거**: 공개 프로필 캘린더의 목적은 계획량보다 실행/완료량을 직관적으로 보여주는 것이며, 기존 대시보드 히트맵의 완료 중심 표현과도 일관된다.
- **결과**: 같은 날짜의 미완료 task(`todo`, `in_progress`)는 셀 강도에 영향을 주지 않고, 완료가 있는 날만 진하게 표시된다.

---

## ADR-022: 공개 프로필 AI 생산성 요약은 인증 사용자 `/me/summary` 우선, 공개 데이터 폴백

- **상태**: 승인
- **결정**: `profile.html`의 AI 생산성 요약은 먼저 `POST /api/v1/profiles/me/summary`를 호출해 응답 `summary`를 사용하고, 미인증/요청 실패 시 기존 `GET /api/v1/profiles/{slug}/tasks` 기반 클라이언트 집계로 폴백한다.
- **근거**: 사용자가 요청한 개인화 요약 엔드포인트(`/me/summary`)를 우선 반영하되, 공개 프로필 페이지의 비로그인 접근성과 내결함성을 유지해야 한다.
- **결과**: 로그인 사용자는 개인 요약을 우선 확인하고, 비로그인 또는 오류 상황에서도 AI 카드가 빈 상태로 멈추지 않고 공개 데이터 기반 요약을 계속 표시한다.

---

## ADR-023: 대시보드 API 호출에 401 기반 자동 토큰 리프레시 래퍼 적용

- **상태**: 승인
- **결정**: 대시보드 템플릿에서 `window.fetch`를 래핑해 API 호출이 401일 때 `/api/v1/auth/refresh`를 1회 수행한 뒤 원 요청을 재시도한다.
- **근거**: access token TTL(15분) 만료 시 기존 구현은 refresh token이 저장되어 있어도 자동 재발급이 없어 사용자가 즉시 로그아웃된 것처럼 동작했다.
- **결과**: refresh token이 유효한 동안 세션이 끊기지 않고, refresh 실패 시 로컬 토큰을 정리해 인증 상태를 일관되게 유지한다.

---

## ADR-024: 공개 프로필 연속 달성일은 API 우선, `done due_date` 기반 클라이언트 폴백

- **상태**: 승인
- **결정**: `profile.html`의 연속 달성일(`stat-streak`)은 `GET /api/v1/profiles/{slug}` 응답의 `streak_days`가 있으면 우선 사용하고, 값이 없으면 `GET /api/v1/profiles/{slug}/tasks?status=done&due_date_to=today`를 페이지네이션 조회해 `due_date` 연속성을 기준으로 계산한다.
- **근거**: 현재 공개 프로필 API 스키마(`stats_summary`)에는 `streak_days`가 필수 필드가 아니며, 화면에서 연속 달성일 0 고정처럼 보이는 문제가 발생할 수 있다. 백엔드 계약 변경 없이도 클라이언트 폴백으로 즉시 사용자 체감 정확도를 보완할 수 있다.
- **결과**: API가 streak를 직접 제공하는 경우와 제공하지 않는 경우 모두에서 연속 달성일이 안정적으로 표시되며, 오늘 완료가 없을 때는 어제부터 연속성을 계산해 자연스러운 streak 동작을 유지한다.

---

## ADR-025: 대시보드 월간 캘린더는 대한민국 공휴일 날짜를 빨간색으로 강조

- **상태**: 승인
- **결정**: `dashboard.html`은 월 렌더 시 대한민국 공휴일 데이터를 연도 단위로 조회해(`https://date.nager.at/api/v3/PublicHolidays/{year}/KR`) 공휴일 날짜 텍스트에 빨간색 스타일을 적용한다.
- **근거**: 기존 캘린더는 주말 색상만 구분되어 실제 공휴일(대체휴일 포함) 인지가 어렵고, 사용자의 일정 계획 정확도를 떨어뜨린다. 백엔드 스키마 변경 없이 프런트 표시 계층에서 해결하는 것이 가장 비용이 낮다.
- **결과**: 월간 heatmap/선택 링 로직은 유지한 채 공휴일 인지가 개선되며, API 실패 시에는 공휴일 강조만 생략하고 기존 캘린더 동작을 유지한다.

---

## ADR-026: Google Tasks import는 공식 필드만 매핑하고 계층/스타/마감일을 단순화

- **상태**: 승인
- **결정**:
  - Google Tasks import는 공식 API 필드(`id`, `title`, `status`, `notes`, `parent`, `deleted`, `hidden`)만 사용한다.
  - `star`, `due`, 그룹 구조는 v1 import에서 반영하지 않는다.
  - 하위 할 일(`parent`)은 계층을 유지하지 않고 1레벨 task로 평탄화한다.
  - 내부 저장 시 `visibility=private`, `group_id=null` 기본값을 사용한다.
  - 중복 방지는 `(user_id, provider, provider_task_id)` 멱등 키로 보장한다.
- **근거**:
  - 현재 도메인 모델은 Google의 계층/스타/마감일/정렬 의미를 1:1로 표현하지 않는다.
  - 초기 단계에서 사용자가 기대하는 핵심은 "기본 할 일을 빠르게 가져오기"이며, 세부 기능 동등성은 요구사항 우선순위가 낮다.
  - 공식 필드 한정 매핑은 비공식 payload 변형에 의한 파싱 리스크를 줄이고 운영 안정성을 높인다.
- **결과**:
